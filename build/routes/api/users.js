 ...
 * 
 * @param   {int}         user_id         A number for identifying each unique user
 * @returns {[Object]}                    An array containing a user object
 */
router.get('/:id', function(req, resp) {
    db('users').where({user_id: req.params.id}).select().then(function(data) {
        resp.send(data);
    });
});
...
router.patch('/:id', function(req, res) {
    db('users').where({ user_id: req.params.id}).update(req.body).returning('*').then(function(data) {
        res.send(data);
    });
...
router.delete('/:id', function(req, res) {
    db('users').where({user_id: req.params.id}).del().then(function() {
